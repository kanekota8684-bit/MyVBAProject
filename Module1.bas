...
' 写真の貼り付け（引数は貼り付ける写真のファイル名）
    Dim myPictureName As String, myPictureNo As Integer, myDate As String, myType As Integer
    ' ファイルネームのパスを取り除いて名前にする
    myPictureName = Right(myFilename, Len(myFilename) - InStrRev(myFilename, "\"))
    ' 写真を取り込みファイルとともに保存する（LinkToFile:=False, SaveWithDocument:=True）
    ' 写真をリンクとして取り込む場合は　LinkToFile:=True, SaveWithDocument:=False　にする
...
' ここに一括取り込み処理
    ...
    With Application.FileDialog(msoFileDialogFilePicker)
        ...
        ' タイトル
        .Title = "画像を選択してください。（「SHIFT」または「CTRL」を押しながらクリックで複数選択可能）"
        ' ボタン表示名
        .ButtonName = "取り込み"
        ' ファイルフィルタを削除
        .Filters.Clear
        ...
        If .Show = -1 Then
            ...
            ' 動的配列変数の上限値を設定
            ReDim myFilename(mySelectedItemsCount - 1)
            ' 配列変数に選択されたファイル名を格納
            For i = 0 To mySelectedItemsCount - 1
                myFilename(i) = .SelectedItems(i + 1)
            Next i
        ' ファイルが選択されなければ（キャンセル）
        Else
            ' 処理を終了
    ...
    If myFnameCount > 0 Then
        ...
        ' 写真の取り込み枚数分くりかえし
        For i = 0 To myFnameCount - 1
            ' 現在のページ番号のセルを選択
            Set myRange = Range(PictureColumn(CurrentNo) & PictureRow(CurrentNo))
            myRange.MergeArea.Activate
            ' 写真の貼り付け
            Call PastePicture(myFname(i))
            ' 次のセルへ移動
            CurrentNo = CurrentNo + 1
            Set myRange = Range(PictureColumn(CurrentNo) & Format(PictureRow(CurrentNo)))
            myRange.MergeArea.Activate
        Next i
        ' 配列変数の解放
        Erase myFname
